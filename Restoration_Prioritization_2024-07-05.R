...
server <- function(input, output, session) {
  ...
  observeEvent(input$protected_checkboxes, {
    ...
    if ("Conservation reserves" %in% input$protected_checkboxes) {
      Protected_areas[CRR_raster == 1] <<- 1
    }

    if ("NGO reserves" %in% input$protected_checkboxes) {
      Protected_areas[NGO_raster == 1] <<- 1
    }

    if ("Natural heritage system areas" %in% input$protected_checkboxes) {
      Protected_areas[NHSA_raster == 1] <<- 1
    }

    if ("Natural heritage value areas" %in% input$protected_checkboxes) {
      Protected_areas[NHVA_raster == 1] <<- 1
    }

    if ("Provincial parks" %in% input$protected_checkboxes) {
      Protected_areas[Parks_raster == 1] <<- 1
    }

    if ("National parks" %in% input$protected_checkboxes) {
      Protected_areas[National_parks_raster == 1] <<- 1
    }

    if ("Conservation areas" %in% input$protected_checkboxes) {
      Protected_areas[CA_raster == 1] <<- 1
    }

    if ("Municipal heritage areas" %in% input$protected_checkboxes) {
      Protected_areas[Municipal_heritage_raster == 1] <<- 1
    }

    if ("Far North protected areas" %in% input$protected_checkboxes) {
      Protected_areas[Far_North_raster == 1] <<- 1
    }

    if ("Wilderness areas" %in% input$protected_checkboxes) {
      Protected_areas[Wilderness_area_raster == 1] <<- 1
    }

    if ("Migratory bird sanctuaries" %in% input$protected_checkboxes) {
      Protected_areas[Migratory_bird_raster == 1] <<- 1
    }

    if ("National wildlife areas" %in% input$protected_checkboxes) {
      Protected_areas[National_wildlife_raster == 1] <<- 1
    }

    if ("National capital valued ecosystems" %in% input$protected_checkboxes) {
      Protected_areas[National_capital_raster == 1] <<- 1
    }

    if ("Provincial planned protected areas" %in% input$protected_checkboxes) {
      Protected_areas[Provincial_plan_protected_raster == 1] <<- 1
    }

    if ("Crown plan protected areas" %in% input$protected_checkboxes) {
      Protected_areas[Crown_plan_protected_raster == 1] <<- 1
    }

    if ("OECMs" %in% input$protected_checkboxes) {
      Protected_areas[OECM_raster == 1] <<- 1
    }

    Protected_areas[OLC == 41] <<- 41 # This adds water for mapping purposes
    movement_cost_protected <<- (movement_cost * 10) # The basic habitat movement cost raster is multiplied by 10, to create the
    # 'areas of conservation concern' cost raster
    movement_cost_protected[Protected_areas == 1] <<- 1 # 'areas of conservation concern' pixels are given a value of 1 in this new raster.
...
# Buffer for target landscape ----
  ...
  observeEvent(input$set_extent, {
    ...
    Protected_areas <- croppedProtected_areas()
    output$protectedPlot <- renderPlot({
      ...
      mapped_values <- factor(land_cover_labels$Land_Class[match(values(Protected_areas), land_cover_labels$Value)])

      # Create a categorical raster and set its categories
      cat_raster <- rast(Protected_areas)
      ...
      values(Protected_areas) <- mapped_values
      plot(Protected_areas, main = "Areas of conservation concern", col = color_palette)
    })
...
#_----
    ...
    movement_cost <- croppedmovement_cost()
    movement_cost[!is.na(test_degraded_pixels_temp)] <- 1000
    movement_cost_raster <- raster(movement_cost)
    tryCatch(
      {
      ...
      error = function(e) {
        ...
        showNotification(paste("Habitat connectivity metrics may not be able to be calculated for the selected landscape due to insufficient nodes (< 5 nodes triggers this warning).
                              Depending on chosen options (degraded land definition and combination type) a successful calculation may still be possible.
                              Alternatively, please add a buffer or select/draw a larger extent so as to encompass additional habitat nodes. You may disregard this warning if only interested
                              in connectivity between nodes of areas of conservation concern."), type = "error", duration = NULL)
      }
    )
    removeNotification(notification_id_extent)
    rm(movement_cost, patches)
    ...
    if (req(input$protected_based_calculations > 0)) {
      ...
      movement_cost <- croppedmovement_cost_protected()
      movement_cost[!is.na(test_degraded_pixels_temp)] <- 10000
      movement_cost_raster <- raster(movement_cost)
      tryCatch(
        {
          patches <- (movement_cost_raster == 1)
  ...
  observeEvent(input$calculate_metrics, {
    ...
    library(furrr)
    plan(multisession(workers = 3)) # Set the number of workers - 3 was most stable on the systems we tested on, but this could be set to anything
  ...
  output$downloadKML <- downloadHandler(
    filename = function() {
      if (!is.null(export_sf())) {
        paste0(input$landscape_name, "top_combination", Sys.Date(), ".kml", sep = "")
      }
    },
    content = function(file) {
      if (!is.null(export_sf())) {
